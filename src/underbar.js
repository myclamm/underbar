...
(function() {
  ...
  _.reject = function(collection, test) {
    return _.filter(collection,function(y){
      return !test(y);
    });
  ...
  _.map = function(collection, iterator) {
    var answer = [];
    _.each(collection,function(x){answer.push(iterator(x))})
    return answer;
  ...
  _.every = function(collection, iterator) {
    if(iterator == undefined){
      return _.reduce(collection,function(base,x){
        return !!x && base
      },true)
    }
  ...
  _.shuffle = function(array) {
    var arrclone = Array.prototype.slice.apply(array);
    var answer = [];
    while(arrclone.length>0){
      var randomnumber = Math.floor(Math.random()*arrclone.length);
      answer.push(arrclone.splice(randomnumber,1)[0])
    }
    return answer;
  };
//Yay
